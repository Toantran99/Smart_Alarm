...
//
	...
	onSwitchRow(rowData, value){
		if(value){
			this.setState({src: require('../assets/bell.png')});
			ToastAndroid.show('Bật báo thức ' + rowData.alarmname + 'thành công!', ToastAndroid.SHORT);
		}
		else{
			this.setState({src: require('../assets/belloff.png')});
	...
	render() {
		...
		return(
			...
			renderRow={(rowData) => {
				...
				{rowData.enable
					? <Image source={require('../assets/bell.png')} style={{margin: 10, width: 35, height: 30, alignItems: 'center', justifyContent: 'center'}} />
					: <Image source={require('../assets/belloff.png')} style={{margin: 10, width: 35, height: 30, alignItems: 'center', justifyContent: 'center'}} />
				}
